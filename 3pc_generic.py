...
运行方式（三个终端，建议先起服务器）:
  python 3pc_generic.py server      # 服务器 (Party 2)
  python 3pc_generic.py hospital_a  # 医院A  (Party 0)
  python 3pc_generic.py hospital_b  # 医院B  (Party 1)
...
class Logger:
    ...
    def __init__(self, mode):
        # 创建日志目录
        self.log_dir = 'nssmpcLog_generic'
...
TARGET_SIZE = (32, 32)  # 改为32×32以匹配蒸馏数据
...
if __name__ == '__main__':
    if len(sys.argv) != 2 or sys.argv[1] not in ('server', 'hospital_a', 'hospital_b'):
        print("用法（三个终端分别执行）:")
        print("  python 3pc_generic.py server      # 第1步：先启动服务器")
        print("  python 3pc_generic.py hospital_a  # 第2步：启动医院A")
        print("  python 3pc_generic.py hospital_b  # 第3步：启动医院B")
        print()
        print("当前配置：")
